...
class AdminService:
    ...
    async def user_delete(
        ...
        try:
            ...
            #         parts = url.split("/")
            #         return "/".join(parts[3:-1])  # Assumes folder structure starts at 4th segment

            #     folder_name = extract_folder_name(user.image)

            #     # Attempt to delete folder from cloud storage (e.g., S3)
            #     deletion = await self.delete_folder_contents(BUCKET_NAME, folder_name)
            #     if not deletion:
            #         raise HTTPException(
            #             status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            # Soft-delete the user (e.g., setting is_deleted = True)
    ...
    async def bloge_delete(
        ...
        try:
            result = await session.execute(
                select(BlogCreate).where(BlogCreate.blog_uid == BlogeID)
            )
            bloge = result.scalars().first()
            if not bloge:
                return {"error": "Bloge not found"}
